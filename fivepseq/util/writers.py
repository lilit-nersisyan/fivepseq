...
class FivePSeqOut:
    ...
    META_COUNT_TERM_PEAKS_FILE = "meta_count_peaks_TERM.txt"
    META_COUNT_START_PEAKS_FILE = "meta_count_peaks_START.txt"
    AMINO_ACID_PAUSES_FILE = "amino_acid_pauses.txt"
    AMINO_ACID_ISLAND_PAUSES_FILE = "amino_acid_island_pauses.txt"
    ...
    def write_transcript_assembly_to_file(self, transcript_assembly, file_name, fivepseq_counts, span_size):
        f = self.open_file_for_writing(file_name)
        if f is not None:
            ...
            for transcript in transcript_assembly:
                f.write('\t'.join([self.get_transcript_attr(transcript, "ID"),
                                   ...
                                   fivepseq_counts.get_cds_sequence_safe(transcript, span_size)
                                   ]) + '\n')
